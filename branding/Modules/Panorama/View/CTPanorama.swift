...
public class CTPanoramaView: UIView {
    ...
    @objc public var controlMethod: CTPanoramaControlMethod = .motion {
        didSet {
    ...
    private lazy var fovHeight: CGFloat = {
        tan(self.yFov / 2 * .pi / 180.0) * 2 * self.radius
    }()
    ...
    @objc
    private func handlePan(panRec: UIPanGestureRecognizer) {
        if panRec.state == .began {
            ...
            var newOrientation = SCNVector3Make(
                                orientation.x + Float(location.y - prevLocation.y) * Float(modifiedPanSpeed.y),
    ...
    @objc
    func handlePinch(pinchRec: UIPinchGestureRecognizer) {
