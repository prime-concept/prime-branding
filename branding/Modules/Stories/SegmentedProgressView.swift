...
class SegmentedProgressView: UIView {
    ...
    private func addProgresses() {
        ...
        for _ in 0 ..< segmentsCount {
            let progressView = SegmentAnimatedProgressView(
                ...
                progressColor: appearance.progressColor
            )
    ...
    func resumeFromBackground(segment: Int, duration: TimeInterval) {
        ...
        progressViews[segment].setFromPaused(duration: duration)
        progressViews[segment].animate(duration: duration, completion: nil)
    }
}
...
private class SegmentAnimatedProgressView: UIView {
    ...
    private func setupView() {
        ...
        topSegmentView.snp.makeConstraints { make in
            topWidthConstraint = make.width.equalTo(self.snp.width).multipliedBy(
    ...
    func setFromPaused(duration: TimeInterval) {
        guard let time = pausedTime else {
            return
        }
        print(duration - time / duration)
        self.set(progress: CGFloat(time.binade))
    }
    ...
    func set(progress: CGFloat) {
        ...
        topSegmentView.snp.makeConstraints { make in
            topWidthConstraint = make.width.equalTo(self.snp.width).multipliedBy(
                progress == 0 ? CGFloat.leastNormalMagnitude : progress
            ).constraint
        }
        topWidthConstraint?.activate()
